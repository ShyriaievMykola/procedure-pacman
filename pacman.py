from constants import WALL, TUNNEL, PELLET, POWER, FRUIT, EMPTY
import pygame
...
old_position: tuple[int, int] = (0, 0)  # Позиція на попередньому кроці
movement_direction: tuple[int, int] = (0, 0)  # Спочатку пекмен стоїть на місці
pending_direction: tuple[int, int] = (0, 0)     
# Напрямок, в який гравець хоче рухатись
...
points_for_ghost: int = starting_points_for_ghost  
# Кількість очок за з'їденого привида
multiplier_for_ghost: float = 2.0  
# Множник очок за кожного наступного з'їденого привида в одному підсиленні


def get_spawn_position(map: GameMap) -> tuple[int, int]:
    """
    Повертає координати спавну пекмена, 
    які визначаються як перша клітинка тунелю, в якій є таблетка
    """
...
def resolve_pend(map: GameMap, ghost_manager: GhostManager | None):
    """
    Розв'язує очікуваний напрямок руху пекмена. 
    Якщо можна рухатись в напрямку очікування - рухаємось в ньому, 
    інакше пробуємо рухатись в поточному напрямку 
    поки не зможемо задовільнити очікування, 
    інакше стоїмо на місці.
    Також з'їдає таблетку в кінці руху, якщо вона є
    """
    maze = map.grid
    global movement_direction, position, old_position
    ...
    else:  # Інакше пробуємо рухатись 
        # в поточному напрямку поки не зможемо задовільнити очікування
...
def eat(position: tuple[int, int], 
        map: GameMap, 
...
def does_touch_ghost(ghost: Ghost):
    ...
    """
    return ghost.position == position or \
        (ghost.position == old_position and ghost.old_position == position)
    # перша частина - дотик якщо в одній клітинці
    # друга частина - дотик якщо пекмен і привид помінялись місцями за крок


def get_touched_ghost(position: tuple[int, int], 
                      ghost_manager: GhostManager) -> list:
    """
...
def eat_pellet(map: GameMap, position: tuple[int, int]):
    """
    З'їдає таблетку в даній клітинці, додає очки, 
    і перевіряє чи не переміг гравець після цього
    """
    global points
    empty_cell(map, position)
    points += 1
    if not any_pellets_left(map):
        victory()  # Перемога, всі таблетки з'їдені


def eat_fruit(map: GameMap, position: tuple[int, int]):
    """
...
def eat_power_pellet(map: GameMap, position: tuple[int, int]):
    """
    З'їдає підсилення в даній клітинці, додає очки, 
    і робить пекмена підсиленим на деякий час
    """
...
def touch_ghost(ghost_manager: GhostManager, ghost: Ghost):
    ...
    робимо пекмена безсмертним на деякий час, 
    і якщо життя закінчились - програємо
    """
...
def game_over():
    """
    Опрацьовує логіку програшу гравця. 
    Відображає екран програшу і чекає на дію гравця
    """
...
def victory():
    """
    Вітаю! ви перемогли! 
    Опрацьовує логіку перемоги гравця. 
    Відображає екран перемоги і чекає на дію гравця
    """
...
def eat_ghost(ghost_manager: GhostManager, ghost: Ghost):
    ...
    З кожним наступним з'їденим привидом в одному підсиленні 
    кількість очок за нього збільшується в кілька разів
    """
...
def empty_cell(map: GameMap, position: tuple[int, int]):
    """
...
def control():
    """
    Визначає напрямок руху пекмена на основі 
    натиснутих клавіш. Зберігає його в pending_direction,
...
def old_control():
    """
    Стара версія control, яка використовує бібліотеку keyboard 
    замість pygame для визначення натиснутих клавіш.
...
def update(map: GameMap, ghost_manager: GhostManager | None):
    """
...
def old_update(map: GameMap, ghost_manager: GhostManager | None):
    """
    Старе оновлення, яке використовує old_control 
    замість control для роботи в консолі
    """
    old_control()
    resolve_pend(map, ghost_manager)
    if empowered:
