...
class TestGetSpawnPosition:
    def test_returns_first_tunnel_with_pellet(self):
        ...
        assert map.pellet_grid[y][x] == PELLET or map.pellet_grid[y][x] == EMPTY
...
class TestMovement:
    ...
    def test_stays_in_place_when_both_directions_blocked(self):
        ...
        pacman.old_position       = (0, 1)
        pacman.pending_direction  = (1, 0)   # right -> WALL at (1,1)
        pacman.movement_direction = (0, 0)   # standing still -> new pos == current pos -> not a wall
        # With (0,0) direction new pos == current pos which is TUNNEL, so pacman stays on same cell
